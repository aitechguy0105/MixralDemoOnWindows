        ...
        if ((int)embd_inp.size() <= n_consumed) {
            ...
            if (n_past > 0 && is_interacting) {
                ...
                if (!params.input_prefix.empty()) {
                    ...
                    // !Caution c_str function convert input_prefix into an array of charaters with a null character at the end.
...
bool CMixtralEngine::loadModel(std::string strModelPath = "", int nMode = 0) {
    ...
    console::init(this->params.simple_io, this->params.use_color);
    atexit([]() { console::cleanup(); });

    if (this->params.logits_all) {
        printf("\n************\n");
    ...
    if (this->params.rope_freq_base != 0.0) {
        LOG_TEE("%s: warning: changing RoPE frequency base to %g.\n", __func__, this->params.rope_freq_base);
    }

    if (this->params.rope_freq_scale != 0.0) {
        LOG_TEE("%s: warning: scaling RoPE frequency by %g.\n", __func__, this->params.rope_freq_scale);
    }
    ...
    std::tie(model, ctx) = llama_init_from_gpt_params(this->params);
    ...
    if (!path_session.empty()) {
        ...
        if (!file_exists(path_session)) {
            ...
            llama_set_rng_seed(ctx, this->params.seed);
    ...
    if (this->params.interactive_first || this->params.instruct || this->params.chatml || !this->params.prompt.empty() || session_tokens.empty()) {
        ...
        if (this->params.chatml) {
            this->params.prompt = "<|im_start|>system\n" + this->params.prompt + "<|im_end|>";
        }
        embd_inp = ::llama_tokenize(ctx, this->params.prompt, add_bos, true);
    }
    else {
    ...
    if (ctx_guidance) {
        ...
        std::vector<llama_token> original_inp = ::llama_tokenize(ctx, this->params.prompt, add_bos, true);
    ...
    if (!session_tokens.empty()) {
        ...
        if (this->params.prompt.empty() && n_matching_session_tokens == embd_inp.size()) {
    ...
    if (this->params.n_keep < 0 || this->params.n_keep >(int) embd_inp.size() || this->params.instruct || this->params.chatml) {
        this->params.n_keep = (int)embd_inp.size();
    }
    else {
        this->params.n_keep += add_bos; // always keep the BOS token
    }
    ...
    if (this->params.interactive_first) {
    ...
    if (this->params.interactive) {
        ...
        if (!this->params.antiprompt.empty()) {
            for (const auto& antiprompt : this->params.antiprompt) {
        ...
        if (!this->params.input_prefix.empty()) {
            ...
            if (this->params.verbose_prompt) {
                auto tmp = ::llama_tokenize(ctx, this->params.input_prefix, true, true);
                for (int i = 0; i < (int)tmp.size(); i++) {
                    LOG_TEE("%6d -> '%s'\n", tmp[i], llama_token_to_piece(ctx, tmp[i]).c_str());
                }
            }
        ...
        if (!this->params.input_suffix.empty()) {
            ...
            if (this->params.verbose_prompt) {
                auto tmp = ::llama_tokenize(ctx, this->params.input_suffix, false, true);
                for (int i = 0; i < (int)tmp.size(); i++) {
                    LOG_TEE("%6d -> '%s'\n", tmp[i], llama_token_to_piece(ctx, tmp[i]).c_str());
                }
            }
    ...
    if (this->params.interactive) {
        ...
        if (this->params.multiline_input) {
...
int CMixtralEngine::releaseModel()
{
    // TODO: Add your implementation code here.
